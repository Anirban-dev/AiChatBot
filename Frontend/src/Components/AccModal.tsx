...
import { X, User, Mail, Lock, Eye, EyeOff, LogOut, RefreshCw, Check, AlertCircle } from 'lucide-react'
...
export default function AccountModal({ isOpen, onClose, user, onLogout, onSwitchAccount, onSave }: AccountModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (tab === 'details') {
        ...
        if (newPassword.length < 8) { setErrorMsg('Password must be at least 8 characters'); setStatus('error'); return }
    ...
    >
      <div className="relative w-full max-w-md mx-4 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex px-6 pt-4 gap-1">
          {(['details', 'credentials'] as Tab[]).map(t => (
            <button
              key={t}
            ...
            >
              {t === 'details' ? 'Account Details' : 'Change Credentials'}
            </button>
          ))}
        </div>
