...
export const useSendMessage = (chatId: string) => {
  ...
  const handleFileUpload = async (
    ...
    try {
      const res = await uploadFile(file, chatId, uploadAbortControllerRef.current.signal)
      if (res.data) {
        onUploaded(res.data)
      }
    } catch (err: any) {
        ...
        async (assistantMsg) => {
          ...
          if (assistantResponses.length <= 1) {
            ...
            const autoGeneratedTitle =
              firstLine.length > 28 ? firstLine.slice(0, 25) + '...' : firstLine

            try {
              await renameChat(targetChatId, autoGeneratedTitle)
              window.dispatchEvent(
                new CustomEvent('chat-auto-renamed', {
  ...
  const handleSendAction = async () => {
    ...
    if (fileToUpload) {
      ...
      await handleFileUpload(fileToUpload, (data) => {
        ...
        appendMessage(chatId, structuredFileMsg)
